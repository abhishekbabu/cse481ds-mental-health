...
# nearest neighbor
m.out <- matchit(matching$household_income_treated ~ matching$Unemployment.raw.value + matching$Some.college.raw.value + matching$RUCC,
...
m.out <- matchit(household_income_treated ~ Unemployment.raw.value + Some.college.raw.value + RUCC,
                 data = matching_sub, method = methods[1], distance = distances[1], replace = TRUE, reuse.max = 3)
summary(m.out)
plot(m.out, type = "jitter", interactive = FALSE)
...
fitmd <- lm(Poor.mental.health.days.raw.value ~ household_income_treated + Unemployment.raw.value + Some.college.raw.value + RUCC, data = gm, 
            weights = weights)
