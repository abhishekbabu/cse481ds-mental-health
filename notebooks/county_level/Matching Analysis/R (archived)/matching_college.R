...
# nearest neighbor
m.out <- matchit(matching$college_treated ~ matching$Unemployment.raw.value + matching$Median.household.income.raw.value + matching$RUCC,
...
fitmd <- lm(Poor.mental.health.days.raw.value ~ college_treated + Unemployment.raw.value + Median.household.income.raw.value + RUCC, data = gm, 
            weights = weights)
